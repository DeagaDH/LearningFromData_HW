...
class LogisticRegression(LearningAlgorithm):
    ...
    def gradient(self,x,y):
        ...
        num = [-y*j for j in x_art]

        #Evaluate denominator (this is a scalar; inner product returns a scalar!)
        den = 1+e**(y*np.inner(self.weights,x_art))

        #Final result
        res = [j/den for j in num]
    ...
    def update_weights(self,eta):
        ...
        for i in range(self.N):
            ...
            #Update weights
            new_weights = [weight - eta*grad for weight,grad in zip(self.weights,gradient)]

            #Insert into self.weights variable
    ...
    def learn(self,eta=0.01,max_iter=1000,tol=0.01):
        ...
        while True:
            ...
            #Check convergence
            norm = np.linalg.norm(np.subtract(self.weights,weights_old))
    ...
    def test_learning(self,x_test,y_test):
        ...
        for i in range(N_test):
            #Add artificial coordinate 1 to x
            x_art=[1]
            x_art.extend(x_test[i])

            #Evaluate error
            error += log(1 + e**(-y_test[i]*np.inner(self.weights,x_art) ))
